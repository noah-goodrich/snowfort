...
## Session 4 — Cortex cost governance rule pack (D1–D6, ~17 rules)
...
### Acceptance criteria
- [ ] `make check` green, coverage ≥ 80%.
- [ ] `cortex_cost.py` module coverage ≥ 90%.
- [ ] Each rule has four test cases: `scan_context=None` → `[]`; view unavailable → `[]` with ERRORED telemetry; threshold crossed → violation; threshold not crossed → `[]`.
- [ ] One Cortex view failing during prefetch does NOT block the others.
- [ ] `snowfort audit rules` lists all 18 new rule IDs.
- [ ] `ParameterizedRuleFamily` implementation is a plain function (no metaclasses) — code review enforcement.
